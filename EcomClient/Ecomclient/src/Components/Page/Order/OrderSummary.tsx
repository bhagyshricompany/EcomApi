...
function OrderSummary({ data, userInput }: orderSummaryProps) {
  ...
  return (
    ...
    toloyalityval = 417.73;
                  ...
                  {/* ${data.cartTotal?.toFixed(2)}  org code */}$
                  {data.cartTotal?.toFixed(2)}
                  {/* {includeLoyalty
                    ? // If loyalty is included, add loyalty value to the cart total
                      `$${(
                        parseFloat(data.cartTotal?.toString() ?? "0") + 417.73
                      ).toFixed(2)}`
                    : // If loyalty is not included, display the cart total as it is
                      `$${parseFloat(data.cartTotal?.toString() ?? "0").toFixed(
                        2
                      )}`} */}
                </h4>
              </div>
